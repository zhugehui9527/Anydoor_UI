...
def Retry(retry_count = 3,isRetry=True):
	...
	def info(func):
		def wrapper(*args, **kwargs):
			...
			L.logger.debug('重跑次数: %s' % str(retry_count))
			L.logger.debug('重跑开关: %s' % str(isRetry))
			if isRetry:
				L.logger.debug('调用函数: %s' % func.__name__)
				casename = result_func[0]  # 返回用例名
				caseresult = result_func[1]  # 返回结果
				
				if result_func:
					for x in range(retry_count):
						L.logger.debug('当前用例名称: %s ,当前执行结果: %s' % (casename, caseresult))
						if caseresult in (resultStutas.fail,resultStutas.error):
							L.logger.debug('失败重跑第 【 %s 】 次执行中' % str(x+1))
							t = func(*args, **kwargs)
							retry_name = t[0]
							retry_result = t[1]
							L.logger.debug('失败重跑执行用例: %s ,执行结果:%s' %(retry_name,retry_result))
						else:
							break
					# return t
				else:

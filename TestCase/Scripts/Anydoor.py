...
class Anydoor_UI(unittest.TestCase):
    ...
    def test_A3(self):
        ...
        self.assertTrue(self.appOperate.check_plugin(pluginId, 'mapSelected'), '插件: %s 断言失败' % pluginId) #进行断言
    ...
    def test_C0(self):
        ...
        logger.debug('遍历插件,插件ID: %s' % pluginId)
        self.assertTrue(self.appOperate.check_plugin(pluginId, '立即购买'), '插件: %s 断言失败' % pluginId)
        #self.appOperate.closeH5_byPluginId(pluginId)
    ...
    # 下一页
    @pytest.mark.skipif(True, reason='PA02100000000_02_QDCS is remaining!')
    def test_C6(self):
        '''用例名称: 检查插件,插件名称: 资产生活,插件ID: PA02100000000_02_QDCS,插件断言: 个人中心 '''
